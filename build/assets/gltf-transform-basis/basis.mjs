...
const basis = function (options) {
  ...
  return async doc => {
    ...
    const promises = textures.map((texture, textureIndex) => limit(async () => {
      ...
      const inPath = tmp.tmpNameSync({
        postfix: '.' + extension
      });
      const outPath = tmp.tmpNameSync({
        postfix: '.ktx2'
      });
      const inBytes = image.byteLength;
      await fs.writeFile(inPath, Buffer.from(image));
      const params = [inPath,  '-output_file' , outPath, ...createParams(slots, channels, size, logger, numTextures, options)];
      console.log(`${prefix}: Spawning → basisu ${params.join(' ')}`); // COMPRESS: Run `toktx` CLI tool.
